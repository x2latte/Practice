...
<template>
  <v-dialog v-model="openDialog" max-width="800" persistent>
    <VForm v-model="formStatus" @submit.prevent="submit">
      <v-card>
        <div v-if="reqId">
          <v-card-title>Редактирование функционального требования (ФТ)</v-card-title>
        </div>
        <div v-else>
          <v-card-title>Добавление функционального требования (ФТ)</v-card-title>
        </div>
        <v-card-text>
          <v-row density="comfortable">
            ...
            >
              <v-select
                ...
                item-title="name"
                item-value="id"
                :items="[{id:0, name:'Высокий'}, {id: 1, name:'Средний'}, {id:2, name:'Низкий'}]"
                label="Приоритет*"
                required
                :rules="[v => validators.integer(v,2)]"
              />
            </v-col>
          </v-row>
          <v-row>
